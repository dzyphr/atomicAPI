...
contract testswapname is ReentrancyGuard
{
        ...
        receive() external payable
	{
        ...
        constructor(address payable rec, uint256 gxX_, uint256 gxY_, uint DURATION) payable
	{
                require(rec != address(0), "receiver is a burn address");
		require(onCurve(gxX_, gxY_) == true, "coordinates are not on sec");
                sender = payable(msg.sender);
                receiver = rec;
                lockHeight = block.number + DURATION;
		gxX = gxX_;
		gxY = gxY_;
        }
        ...
        function senderReclaim() external 
	{
                ...
                require(block.number >= lockHeight, "timelock has not expired!");
                selfdestruct(sender);
        }
